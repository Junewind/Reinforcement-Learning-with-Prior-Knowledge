...
class KellyCoinflipEnv(gym.Env):
    ...
    def __init__(self, initial_wealth=20.0, edge=0.62, max_wealth=150.0, max_rounds=10):

        #self.action_space = spaces.Discrete(int(max_wealth * 100))  # betting in penny
        self.action_space = spaces.Discrete(int(4))  # no bet, 20% bet, 50% bet, all-in
        # increments
        '''
        ...
        '''
        self.observation_space = spaces.Discrete(int(max_wealth//5) + 1)
        self.reward_range = (0, max_wealth)
        self.edge = edge
        self.wealth = initial_wealth
        self.past_wealth = initial_wealth          # use this to 
    ...
    def step(self, action):
        ...
        bet_in_dollars = 0
        if action==0:
            bet_in_dollars = int(self.wealth*0.1)
        elif action==1:
            bet_in_dollars = int(self.wealth*0.2)
        elif action==2:
            bet_in_dollars = int(self.wealth*0.4)
        elif action==3:
            bet_in_dollars = int(self.wealth*0.6)
        self.rounds -= 1

        coinflip = flip(self.edge, self.np_random)
        
        self.past_wealth = int(self.wealth)
        if bet_in_dollars != 0:
            self.wealth = int(min(self.max_wealth, self.wealth + coinflip * bet_in_dollars))
        else:
            self.wealth = math.floor(self.wealth*0.85)

        done = self.wealth < 0.01 or not self.rounds or self.wealth == self.max_wealth
        reward = self.wealth if done else 0.0
        #reward = (self.wealth - self.past_wealth) if not done else self.wealth
        
        return self._get_obs(), reward, done, {}

    def _get_obs(self):
    #    return int(self.wealth), self.rounds
        return int(self.wealth//5)

    def reset(self):
        self.rounds = self.max_rounds
        self.wealth = self.initial_wealth
        self.past_wealth = self.initial_wealth
...
class KellyCoinflipGeneralizedEnv(gym.Env):
    ...
    def __init__(self, initial_wealth=25.0, edge_prior_alpha=7, edge_prior_beta=3,
                 max_wealth_alpha=5.0, max_wealth_m=200.0, max_rounds_mean=30.0,
